...
export async function getLiveData(): Promise<LiveData> {
  const res = await fetch(`${CONFIG.API_BASE_URL}/api/live-data`);
...
export async function triggerAlert(
  severity: string,
  title: string
): Promise<{ message: string; alert: AlertInfo }> {
  const res = await fetch(`${CONFIG.API_BASE_URL}/api/alerts/trigger`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
